...
export default function LanguageToggle() {
  ...
  return <>
    ...
    {open && <>
      <button aria-label={lang === "ru" ? "Закрыть меню" : "Мәзірді жабу"} type="button" onClick={() => setOpen(false)} style={{position:"fixed",inset:0,zIndex:99,border:0,background:"rgba(0,0,0,.52)"}} />
      <aside style={{position:"fixed",top:0,right:0,zIndex:100,width:"min(330px,88vw)",height:"100dvh",padding:"78px 20px 24px",background:"linear-gradient(180deg,#e10600 0%,#a80400 42%,#700300 100%)",borderLeft:"1px solid #ff4a44",boxShadow:"-18px 0 50px rgba(0,0,0,.5)",display:"flex",flexDirection:"column",gap:16}}>
        <div><div style={{color:"rgba(255,255,255,.7)",fontSize:11,fontWeight:900,letterSpacing:".12em"}}>{lang === "ru" ? "МЕНЮ" : "МӘЗІР"}</div><h3 style={{margin:"7px 0 0",fontSize:22,color:"#fff"}}>{lang === "ru" ? "Настройки" : "Баптаулар"}</h3></div>
