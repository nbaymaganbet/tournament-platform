...
export default function ParticipantsClient({ tournamentId, initialRows }: { tournamentId:string; initialRows:Row[] }) {
  ...
  const labels=locale==="kk"?{edit:"Өзгерту",delete:"Өтінімді жою",menu:"Әрекеттер",confirm:"Өтінімді растау",confirmed:"Өтінім расталды",deleteConfirm:"Бұл өтінімді жою керек пе?",save:"Сақтау",cancel:"Бас тарту",editTitle:"Өтінімді өзгерту",first:"Аты",last:"Тегі",age:"Жасы",weight:"Салмағы",experience:"Тәжірибесі",phone:"Телефон",club:"Клуб",coach:"Жаттықтырушы"}:{edit:"Изменить",delete:"Удалить заявку",menu:"Действия",confirm:"Подтвердить заявку",confirmed:"Заявка подтверждена",deleteConfirm:"Удалить эту заявку?",save:"Сохранить",cancel:"Отмена",editTitle:"Изменить заявку",first:"Имя",last:"Фамилия",age:"Возраст",weight:"Заявленный вес",experience:"Опыт",phone:"Телефон",club:"Клуб",coach:"Тренер"};
  ...
  async function confirmApplication(id:string){setBusy(id);setOpenMenu(null);const {data,error}=await supabase.rpc("confirm_tournament_registration",{p_registration_id:id});if(!error&&data)setRows(c=>c.map(r=>r.id===id?{...r,status:"confirmed",payment_status:"paid"}:r));if(error)window.alert(error.message);setBusy(null);}
