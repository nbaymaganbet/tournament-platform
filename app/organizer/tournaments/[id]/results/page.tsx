...
export default async function ResultsPage({params}:{params:Promise<{id:string}>}){
 ...
 const{data:results,error}=await supabase.from("results").select("id,place,category_id,participant_id,categories(name),undefined").eq("tournament_id",id).order("category_id").order("place");if(error)throw new Error(error.message);
