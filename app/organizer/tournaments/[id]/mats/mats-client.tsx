...
export default function MatsClient({tournamentId,initialMats}:{tournamentId:string;initialMats:Mat[]}){
 ...
 async function rename(m:Mat){const next=prompt("Название ковра",m.name)?.trim();if(!next||next===m.name)return;const{error}=await s.from("mats").update({name:next}).eq("id",m.id).eq("tournament_id",tournamentId);if(!error)setMats(x=>x.map(a=>a.id===m.id?{...a,name:next}:a));else alert(error.message)}
 ...
 async function remove(m:Mat){if(!confirm("Удалить ковёр?"))return;const{error}=await s.from("mats").delete().eq("id",m.id).eq("tournament_id",tournamentId);if(!error)setMats(x=>x.filter(a=>a.id!==m.id));else alert(error.message)}
 return <section><form className="toolbar" onSubmit={add}><input className="field" value={name} onChange={e=>setName(e.target.value)} placeholder="Название, например Ковёр 1"/><button className="primary" disabled={busy}>{busy?"Добавляем…":"Добавить ковёр"}</button></form><div className="category-list">{mats.length===0?<div className="empty-state">Ковров пока нет.</div>:mats.map(m=><article className="participant-card" key={m.id}><div className="participant-main"><strong>{m.name}</strong><span className="status">{m.is_active?"Активен":"Выключен"}</span></div><div className="participant-actions"><button type="button" className="secondary" onClick={()=>rename(m)}>Переименовать</button><button type="button" className="secondary" onClick={()=>toggle(m)}>{m.is_active?"Выключить":"Включить"}</button><button type="button" className="danger-button" onClick={()=>remove(m)}>Удалить</button></div></article>)}</div></section>
}
