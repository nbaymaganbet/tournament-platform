...
export default async function MatsPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 return <main className="container dashboard-page"><div className="page-topline"><Link className="back-link" href={`/organizer/tournaments/${id}`}>← {t.name}</Link></div><div className="section-header"><div><div className="eyebrow">ПРОВЕДЕНИЕ</div><h1>Ковры</h1><p className="muted">Создайте площадки, на которых будут проходить поединки.</p></div></div><MatsClient tournamentId={id} initialMats={mats??[]}/></main>;
}
