...
export default async function TournamentPage({params}:{params:Promise<{id:string}>}){
  ...
  const {data:schedule}=await s.from("match_schedule").select("match_id,scheduled_order,approximate_time,mats(name)").in("match_id",(matches??[]).map(m=>m.id)).order("scheduled_order");
  ...
  const deadline=t.registration_deadline?` · до ${new Date(t.registration_deadline).toLocaleDateString("ru-RU")}`:"";
