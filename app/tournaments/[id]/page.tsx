...
export default async function TournamentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: tournament } = await s.from("tournaments").select("id,name,date,city,venue,sport,description,entry_fee,registration_deadline,status,poster_url,is_public,regulations_text").eq("id", id).eq("is_public", true).maybeSingle();
  if (!tournament) notFound();
  const [{ data: cats }, { data: matches }, { data: results }] = await Promise.all([
  ...
  return <main className="container public-tournament">
    ...
    <header className="public-hero">{tournament.poster_url && <img src={tournament.poster_url} alt="" />}<div><div className="eyebrow">{tournament.sport} · {tournament.city}</div><h1>{tournament.name}</h1><p className="muted">{tournament.date ? new Date(tournament.date).toLocaleDateString(dateLocale) : "Дата уточняется"} · {tournament.venue || text.placeUnknown}</p>{tournament.description && <p>{tournament.description}</p>}{registrationOpen && <Link className="primary button-link" href={`/tournaments/${id}/register`}>{text.register}</Link>}</div></header>
    ...
    <section className="public-section regulations-card">
      <div className="section-header">
        <div><h2>{text.regulations}</h2><p className="muted">{tournament.regulations_text ? (ru ? "Полный текст положения доступен на сайте." : "Ереженің толық мәтіні сайтта қолжетімді.") : (ru ? "Положение пока не опубликовано." : "Ереже әлі жарияланбаған.")}</p></div>
        {tournament.regulations_text && <div className="action-row"><Link className="primary button-link" href={`/tournaments/${id}/regulations`}>{text.openRegulations}</Link><ShareEventButton title={`${tournament.name} — ${text.regulations}`} locale={locale} url={`/tournaments/${id}/regulations`} /></div>}
      </div>
    </section>
  </main>;
}
