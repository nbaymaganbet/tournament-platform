...
returns void language plpgsql set search_path to public,private
as $$
declare
...
begin
 ...
 if m.next_match_id is not null then
   ...
   update public.matches set participant_a_id=case when participant_a_id is null then p_winner_id else participant_a_id end,participant_b_id=case when participant_a_id is not null and participant_b_id is null then p_winner_id else participant_b_id end,updated_at=now() where id=m.next_match_id;
 end if;
 if m.loser_next_match_id is not null and loser_id is not null then
   update public.matches set participant_a_id=case when participant_a_id is null then loser_id else participant_a_id end,participant_b_id=case when participant_a_id is not null and participant_b_id is null then loser_id else participant_b_id end,updated_at=now() where id=m.loser_next_match_id;
 end if;
 if exists(select 1 from public.matches x where x.bracket_id=m.bracket_id and x.round_number=1 and x.match_number=1)
   ...
   if final_m.id=m.id and first_m.status='completed' then
     bye_id:=case when final_m.participant_a_id not in (first_m.participant_a_id,first_m.participant_b_id) then final_m.participant_a_id else final_m.participant_b_id end;
     ...
     if p_winner_id=bye_id then
       ...
       insert into public.results(tournament_id,category_id,participant_id,place) values(m.tournament_id,m.category_id,loser_id,3) on conflict(category_id,participant_id) do update set place=excluded.place;
     else
 ...
 select * into final_m from public.matches where bracket_id=m.bracket_id and next_match_id is null and loser_next_match_id is null and participant_a_id is not null and participant_b_id is not null order by round_number desc,match_number asc limit 1;
 if final_m.id=m.id then
   insert into public.results(tournament_id,category_id,participant_id,place) values(m.tournament_id,m.category_id,m.winner_id,1) on conflict(category_id,participant_id) do update set place=excluded.place;
   if loser_id is not null then insert into public.results(tournament_id,category_id,participant_id,place) values(m.tournament_id,m.category_id,loser_id,2) on conflict(category_id,participant_id) do update set place=excluded.place; end if;
 end if;
 select * into third_m from public.matches where bracket_id=m.bracket_id and id<>coalesce(final_m.id,'00000000-0000-0000-0000-000000000000'::uuid) and next_match_id is null and loser_next_match_id is null order by round_number desc,match_number asc limit 1;
 if third_m.id=m.id then insert into public.results(tournament_id,category_id,participant_id,place) values(m.tournament_id,m.category_id,m.winner_id,3) on conflict(category_id,participant_id) do update set place=excluded.place; end if;
end;
$$;